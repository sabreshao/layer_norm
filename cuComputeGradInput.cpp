...
void HostLayerNormGradient(
  ...
  if (mean == nullptr && !simplified) {
    ...
    hipLaunchKernelGGL(HIP_KERNEL_NAME(cuComputePartGradGammaBeta<T, U, false, false>), dim3(blocks2), dim3(threads2), nshared2, stream, 
      dout,
      input,
      output,
      gamma,
    ...
    hipLaunchKernelGGL(HIP_KERNEL_NAME(cuComputePartGradGammaBeta<T, U, true, simplified>), dim3(blocks2), dim3(threads2), nshared2, stream, 
      dout,
      input,
      output,
      gamma,
  ...
  hipLaunchKernelGGL(HIP_KERNEL_NAME(cuComputeGradGammaBeta<T, U, simplified>), dim3(blocks3), dim3(threads3), nshared3, stream, 
      part_grad_gamma,
      part_grad_beta,
      part_size,
      n1, n2,
...
#define LAYERNORMGRAD_IMPL(T, U, simplified)                                                                                                  \
  template void HostLayerNormGradient<T, U, simplified>(const hipDeviceProp_t& prop, hipStream_t stream, const T* dout, const T* input, const T* output,           \
